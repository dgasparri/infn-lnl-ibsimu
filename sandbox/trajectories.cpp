     ...
     )
{
    ...
    for( int a = 0; a < n_rounds; a++ ) {
        ...
        if( a == 1 ) {
            electron_charge_density_rhoe = pdb.get_rhosum();
        ...
        /*
        int tr_num_particles = pdb.size();
        std::cout<<"Num parts: "<< tr_num_particles <<std::endl;
        for(int xi = 0; (xi < tr_num_particles) && (xi<5) ; xi++) {
            const Particle<ParticlePCyl> &par = pdb.particle(xi);
            const int tr_traj_size = par.traj_size();
            Vec3D loc = par.location();
            Vec3D vel = par.velocity();
            const ParticlePCyl pp=par.x();

            std::cout<<"(Particle, traj_size):  "<< xi << ", " <<tr_traj_size <<std::endl;
            std::cout<<"   (loc),(vel): "
                        << ",(" 
                        << loc << "),("<< vel <<")" <<std::endl;
            std::cout<<"    (ParticlePoint): "
                    << pp <<std::endl;

            
            std::cout<<std::endl;
            for(int xj =0; (xj < tr_traj_size) && (xj <3); xj++ ) {
                const ParticlePCyl &particlePoint = pdb.trajectory_point(xi,xj);
                double t = particlePoint[0];
                Vec3D loc = particlePoint.location();
                Vec3D vel = particlePoint.velocity();
                std::cout<<"    t,(loc),(vel): "
                        << t << ",(" 
                        << loc << "),("<< vel <<")" <<std::endl;
        ...
        */
        ...
        if (a==20) {
            ...
            ParticleDataBaseCyl pdb2( geometry_o );
            pdb2.set_mirror( pmirror );

            int tr_num_particles = pdb.size();
            //Particle<ParticlePCyl> *particles[tr_num_particles];
            for(int xi=0; xi<tr_num_particles; xi++) {
                const Particle<ParticlePCyl> &par = pdb.particle(xi);
                pdb2.add_particle(
                    par.IQ(), 
                    par.q(), 
                ...
                //Particle(IQ,q,m,PP &x);
                /*particles[xi] = new Particle<ParticlePCyl>(
                    par.IQ(), 
                    par.q, 
                    par.m, 
                    par.traj(0) );
            ...
            pdb2.save("pdb2-uguale-pdb-init.dat");
            std::cout<<"electron_charge_density_rhoe: "<<electron_charge_density_rhoe <<std::endl;
            break;
